...
| `crop_opt`      | integer for using either input data  (= `0`, default) or user set crop vegetation type heights from namelist (= `1`).  Note: use of can_opt=1 will only use `crop_chset`, `crop_cfset`, or `crop_laiset` values if observations are found to be missing, i.e., <=0.0.|  |
